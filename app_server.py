...
def get_violate():
    ...
    cur.execute("""
        SELECT nametransportation.vh_name, 
               COUNT(transportationviolation.id) AS total_violate 
        FROM transportationviolation 
        INNER JOIN nametransportation ON transportationviolation.id_name = nametransportation.id_name 
...
def get_violate_current():
    cur = mysql.connection.cursor()
    # truy vấn để tính số lần vi phạm trong ngày hiện tại
    cur.execute("""
        SELECT nametransportation.vh_name, 
               COUNT(transportationviolation.id) AS total_violate 
        FROM transportationviolation 
                        ...
                        if box.cls[0] == 1:
                            if canh_bao_vi_pham_lane_oto:
                                ...
                                if filterData:
                                    ...
                                    # cv2.imwrite(".\python_project\data_xe_may_vi_pham\ " + str(count) + ".xe_may_lan_lan.jpg",
                                    #             cropped_frame)
...
@app.route("/camera1")
def video():
    return Response(generate_frames(path_x="./Videos/lane.mp4"),
                    mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route("/camera2")
def video_2():
    return Response(generate_frames_helmet(path_x="./Videos/test13.mp4"),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
